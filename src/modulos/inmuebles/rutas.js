...
router.get('/', todos);
router.get('/infoInmueblesContratos', inmueblesContratos);
router.get('/:id', uno);
router.post('/', seguridadRol(), agregar);
router.delete('/', seguridadRol(), eliminar);

// Función para manejar la obtención de todos los inmuebles
...
// Función para manejar la obtención de todos los inmuebles con información de contratos y contactos relacionados
async function inmueblesContratos(req, res, next) {
	try {
		const items = await controlador.inmueblesContratos();
		respuesta.success(req, res, items, 200);
	} catch (err) {
		next(err);
	}
